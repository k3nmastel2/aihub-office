...
| `src/features/retro-office/systems/visualSystems.tsx` | (T12c) `TrailSystem` `setPoints` guarded by reference-equality on the point set — was publishing a fresh array (empty when trailMode off) at ~7.5Hz in `useFrame`, unconditionally mounted → the re-render engine driving the residual update-depth bursts. **Measured 32→0/60s, clean 3-min soak.** The SOLE measurement-implicated fix (static-sweep steps #2-4 not needed). | 1/T12c |
