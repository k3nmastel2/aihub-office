...
| `src/features/office/screens/OfficeScreen.tsx` | (T12c) **stabilize `officeTriggerState` by VALUE for the `officeAnimationState` memo** (`stableOfficeTriggerState` = `useMemo(…, [JSON.stringify(officeTriggerState)])`) — the DRIVER, named via per-dep diff under active churn: `officeTriggerState` identity churned ~50×/cascade (1256/60s) poisoning the memo while `state.agents` stayed flat. Also gym-cooldown hardening (`animationNowMs` quantized to 1s buckets; `prevImmediateGymHoldRef` mutation moved out of the pure updater + quantized latch `now`) — correct hardening, though gym proved a BYSTANDER (setState *called* ~50×/cascade but always bails; the 91% stack-decode was a frequency artifact). **The value-stabilization is INSUFFICIENT** (kept as documented hardening, NOT a verified fix): under active churn (roster 19) the loop persists at **27/60s** — either officeTriggerState's VALUE churns 50×/cascade (JSON key churns too) or the churn hops to another memo dep. NOTE: the earlier TrailSystem guard (commit c92bff0) was REVERTED — churn-variance false positive. **T12 = CONTAINMENT / P2 debt** — no verified fix; full evidence in `docs/aihub/T12-STATIC-SWEEP.md` + PROGRESS.md T12c. | 1/T12c (debt) |
