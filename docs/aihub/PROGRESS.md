...
| T28 | P2/P3 | **ROOT-CAUSED 2026-07-11 (phase1-provider) — SAME BUG AS T22.** The recurring zero-arg "Uncaught (in promise)" is an UNHANDLED PROMISE REJECTION from `WebAssembly.instantiate()` blocked by the prod CSP. Captured via a doc-start `unhandledrejection` listener (fires ~145ms into load): `CompileError: WebAssembly.instantiate(): ... violates ... "script-src 'self' 'unsafe-inline' blob:"`. The chrome-devtools console renders the promise-rejection as "(0 args)" — hence "zero-arg/no-stack". **Chain:** `next.config.ts:17-19` includes `'unsafe-eval'` in dev (WASM ok) but drops it in prod (comment wrongly assumes "three.js doesn't need eval"); drei's GLTF loader instantiates a Meshopt/Draco **WASM decoder** via `useGLTF.preload` (`retro-office/objects/furniture.tsx:754`) at module-eval → prod CSP has neither `'unsafe-eval'` nor `'wasm-unsafe-eval'` → CompileError → unhandled rejection. Prod-only, ~once/session on load. Empirically NOT a fetch leak (broke all `/api/` fetch during board/panel open → 0 rejections; every fetch site catches). **FIX (one line, closes T28 + T22):** `next.config.ts:19` → `["script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval' blob:"]` (`'wasm-unsafe-eval'` enables WASM compile without re-enabling general `eval()` — keeps the anti-XSS posture). **APPLIED + VERIFIED 2026-07-11 (team-lead approved):** prod rebuilt on :3100 (CSP header now serves `'wasm-unsafe-eval'`); a fresh IIFE-interceptor reload → **0 unhandled rejections + 0 WASM CompileErrors** across a 25s soak (was firing at 145ms every load). **Bonus check:** furniture renders IDENTICALLY before/after (evidence screenshots) → the GLTF assets are uncompressed, so the decoder was speculative and the block was pure console noise — no missing meshes. Recipe/method note: devtools `initScript` must be an IIFE `(()=>{...})()` — a bare arrow fn silently no-ops (that gotcha is why the first capture attempts came up empty). | **FIXED + verified (T22 + T28)** |
