...
| T15 | P1 | **Walk-out+fade did NOT fire on a live done node (Phase 2 gate, ghost avatar reproduced):** "Plan" capture agent flipped active→winding→done and stayed fully visible/opaque 5+ min (evidence: `evidence/phase2/24-walkout-fade-NOT-OBSERVED.png`), though fade+hide was verified twice pre-gate on sub-explore. Hypotheses: (a) the T17 disconnect/re-hydrate overlapped the done-flip — check whether leaving derivation survives a reconnect (hub metadata refresh, firstSeen ref reset, leavingSince latch); (b) walk-out queue starvation — cap of 4 slots may be occupied by stuck/legacy done nodes whose walk-out never completes, blocking new leavers. Owner: phase2-lifecycle. Blocks Phase 2 close. | routed |
| T16 | P1→classify | **Floor-switch roster carryover:** switching to Lobby/DEMO leaves all 21 AI Hub avatars rendered + header "21 AGENTS", while the floor panel correctly shows roster 0 — global roster/scene not reset to the selected floor. Prevented the adapter-gating spawn check. Determine pre-existing upstream (floorRosterCache/scene reuse) vs Phase 2 regression; if pre-existing, reclassify P2 upstream debt. Owner: phase2-lifecycle (classification + fix if ours). | routed |
| T17 | P1-investigate | **Mid-session disconnect + Context Lost + 502×72 burst while STATIONARY on AI Hub Live** (not floor-switching — distinct from accepted dead-backend-floor 502s), auto-recovered to CONNECTED. Threads: hub-side availability blip (background ingest? load?) and client-side reconnect behavior — esp. WHY Context Lost accompanied the disconnect (canvasResetKey no longer keys on gatewayStatus; what else remounts the canvas on connection loss?). May explain T15 hypothesis (a). **Hub-side check (orchestrator):** hub healthy (200/1.6ms); the session-disappear ingest is `asyncio.to_thread`-wrapped (app.py:4011) → hub-blocking hypothesis weakened; hub+harvester file logs stopped writing 20:08 (server restarted with logs elsewhere?) so no direct window evidence. Leading suspect: the NEXT DEV SERVER stalling (HMR + multi-tab + compile pressure) — the 502s originate from our proxy's upstream fetch failing. Another dev-harness confound; one more argument for the production-build follow-up. | routed |
